...
client.user.setGame(`=sh | By ShAnKs`,"http://twitch.tv/ShAnKs")
...
client.on("message", async message => {
    if(message.content.startsWith(prefix + "help")) {
        let help = new Discord.RichEmbed()
            ...
            رابط سيرفر الدعم الفني : https://discord.gg/JYRujGN
            **`);
...
client.on("ready", () => {
    var guild;
    while (!guild)
        guild = client.guilds.get("541660472710135811");
...
client.on("guildMemberAdd", (member) => {
    let channel = member.guild.channels.get("541660472710135823");
    if (!channel) {
 ...
 channel.send(`${Invite} تم دعوته بواسطة ${Invite.inviter} `) ;         
 }
            dat[Inv] = Invite.uses;
